  ...
  const generateAIForecast = async (wasteType = null) => {
    try {
      ...
      for (const type of typesToForecast) {
        ...
        try {
          // Use dynamic model discovery like other components
          console.log('🔍 Discovering working Gemini model for AIWasteForecast...');
          const { getWorkingGeminiModel } = await import('../utils/geminiModelDiscovery.js');
          const workingModelName = await getWorkingGeminiModel();
          
          console.log(`✅ Using discovered model: ${workingModelName}`);
          
          let response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${workingModelName}:generateContent?key=${geminiApiKey}`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            // Handle 503 Service Unavailable errors by trying a different model
            if (response.status === 503) {
              console.log('🔄 Model unavailable (503), clearing cache and retrying...');
              
              const { clearModelCache, getWorkingGeminiModel } = await import('../utils/geminiModelDiscovery.js');
              clearModelCache();
              
              const newWorkingModel = await getWorkingGeminiModel();
              console.log(`🔄 Retrying with new model: ${newWorkingModel}`);
              
              response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${newWorkingModel}:generateContent?key=${geminiApiKey}`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (!response.ok) {
                throw new Error(`Gemini API error after retry: ${response.status}`);
              }
            } else {
