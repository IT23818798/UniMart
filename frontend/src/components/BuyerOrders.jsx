...
const BuyerOrders = ({ buyer, onOrderClick }) => {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    return (
      ...
      String(order.seller?.businessName || '').toLowerCase().includes(q) ||
      String(order.orderStatus || '').toLowerCase().includes(q) ||
      String(order.contactPhone || '').toLowerCase().includes(q) ||
      items.some((item) => String(item.title || '').toLowerCase().includes(q))
    );
  });
  ...
  const handleDownloadPDF = (order, e) => {
    ...
    doc.save(`Unimart_Receipt_${String(order._id || '').substring(0, 8)}.pdf`);
  };
          ...
          <tbody className="divide-y divide-gray-200 text-sm">
            {filteredOrders.map((order) => {
              ...
              return (
                ...
                >
                  <td className="py-3 px-4 font-mono text-xs">{String(order._id).substring(0, 8)}...</td>
                  <td className="py-3 px-4">{order.seller?.businessName || 'Unknown'}</td>
                  <td className="py-3 px-4">
                    {items.map((item, index) => (
                      <div key={`${order._id}-${index}`} className="text-gray-800">
                  ...
                  <td className="py-3 px-4">
                    <span
                      className={`px-3 py-1.5 rounded-full text-xs font-bold capitalize shadow-sm border ${
                  ...
                  <td className="py-4 px-5 flex items-center gap-3">
                    <button
                      ...
                      className="px-3 py-1.5 bg-green-50 text-green-600 border border-green-200 rounded-lg text-sm font-semibold hover:bg-green-100 transition-all flex items-center gap-1.5"
                    ...
                    {order.orderStatus !== 'cancelled' && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-white text-red-500 border border-red-100 rounded-lg text-sm font-semibold hover:bg-red-50 transition-all flex items-center gap-1.5"
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-blue-50 text-blue-600 border border-blue-200 rounded-lg text-sm font-semibold hover:bg-blue-100 transition-all"
                      title="Edit Order"
                    >
                      Edit
                    </button>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleUpdateOrder} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" onClick={() => setEditingOrder(null)} className="px-4 py-2 border rounded-md text-gray-600 hover:bg-gray-50">
                  Cancel
                </button>
